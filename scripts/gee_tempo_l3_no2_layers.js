/**
 * Google Earth Engine — TEMPO NO2 L3 (QA-filtered) over a configurable rectangle
 *
 * Open: https://code.earthengine.google.com/
 * Paste this script → Run. Edit STUDY_AOI, DATE_START, DATE_END for your case.
 *
 * Collection: NASA/TEMPO/NO2_L3_QA
 * See: https://developers.google.com/earth-engine/datasets/catalog/NASA_TEMPO_NO2_L3_QA
 *
 * Note: L3 is gridded (~2.2 km); not identical to L2 NetCDF swath geometry.
 */

// Rectangle: [minLon, minLat, maxLon, maxLat] in WGS84 (example: SoCal / LA basin)
var STUDY_AOI = ee.Geometry.Rectangle({
...
// One composite for the window (mean of L3 snapshots touching the AOI).
var no2Mean = col.select('vertical_column_troposphere').mean().clip(STUDY_AOI);
var no2Unc = col.select('vertical_column_troposphere_uncertainty').mean().clip(STUDY_AOI);
var cloud = col.select('eff_cloud_fraction').mean().clip(STUDY_AOI);
var weight = col.select('weight').mean().clip(STUDY_AOI);
...
Map.centerObject(STUDY_AOI, 10);
Map.addLayer(STUDY_AOI, { color: 'white' }, 'AOI outline', true, 0.4);

Map.addLayer(no2Mean, visNo2, 'NO2 troposphere (mean)', true, 0.85);
...
// Optional: unfiltered sibling collection for comparison
// var colRaw = ee.ImageCollection('NASA/TEMPO/NO2_L3')
//   .filterBounds(STUDY_AOI).filterDate(DATE_START, DATE_END);
// Map.addLayer(colRaw.select('vertical_column_troposphere').mean().clip(STUDY_AOI),
